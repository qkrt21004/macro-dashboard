...
with tab_cal:
    ...
    # Legend
    leg_cols = st.columns(len(LEGEND))
    for col, (label, color) in zip(leg_cols, LEGEND):
        col.markdown(
...
with tab_dash:
    ...
    with m4:
        if sprd_v is not None: st.metric("2Y–10Y Spread", f"{sprd_v:+.2f}%", f"{sprd_d:+.2f}pp")
    with m5:
    ...
    with r4d:
        ...
        st.caption("Source: OECD via FRED (IRLTLT01JPM156N) · Monthly")
    ...
    st.caption("Source: ICE BofA via FRED (BAMLH0A0HYM2)")
